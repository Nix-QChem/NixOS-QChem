...
batsTest {
  ...
  testScript = ''
    ...
    @test "CASPT2 Energy" {
      grep '!RSPT2 STATE 1.1 Energy' molpro.out | tail -1 | grep '\-114.3045186'
    }

    @test "MRCI Energy" {
      grep '!MRCI STATE 1.1 Energy' molpro.out | grep '\-114.2818465'
      grep '!MRCI STATE 2.1 Energy' molpro.out | grep '\-114.0268171'
    }
  '';
}
